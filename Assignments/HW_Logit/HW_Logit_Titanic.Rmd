...
for(i in 1:length(pred.list))
{
  actual <- survived[i]
  pred <- pred.list[i]
  cm[(pred+1), actual] <- cm[(pred+1), actual] + 1
}
...
---- | ---- | ---- | ---- | ----
`r precision` | `r recall` | `r sensitivity` | `r specificity` | `r f1`  

Looking at the `Specificity` and the `F1` scores, I would not say it is a good model.
...
Compared to the previous models, almost all evaluation metrics including AIC have improved. Also the ROC curve appears good now. Hence, the model appears to be better than the previous model.  
