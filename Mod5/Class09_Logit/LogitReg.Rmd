...
A quick summary, these three ways all calculate the same thing, which is the predicted values from the model. 

* This gives you the fitted value for *p* directly: `admitLogit$fitted.values[1]` = `r admitLogit$fitted.values[1]`  
* Or you can first calculate the predicted value for ln(*p*/*q*): `predict(admitLogit)[1]` = `r predict(admitLogit)[1]`  
Then find the *p* value from: `1/(1+exp(-predict(admitLogit)[1]))` = `r 1/(1+exp(-predict(admitLogit)[1]))`
* Or use the built-in type option in predict( ) to get the *p* value: `predict(admitLogit, type = "response")[1]` = `r predict(admitLogit, type = "response")[1]`
